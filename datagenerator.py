...
class DataGenerator:
    ...
    def generate_names(self, nr):
        rnd_fn = self.generate_first_names(nr)
        rnd_ln = self.generate_last_names(nr)
        result = [{"first_name": rnd_fn[x], "last_name":rnd_ln[x]}
                  for x in range(nr)]
    ...
    def generate_first_names(self, nr):
        ...
        result = [self.__first_names[random.randint(
            0, fn_length-1)] for x in range(nr)]
        return result

    def generate_last_names(self, nr):
        ln_length = len(self.__last_names)
        result = [self.__last_names[random.randint(
            0, ln_length-1)] for x in range(nr)]
        return result
    ...
    def generate_projects(self, nr, serialize=False):
        ...
        for _ in range(nr):
            ...
            start_date = datetime.datetime(random.randint(
                1990, 2015), random.randint(1, 12), random.randint(1, 28))
            end_date = start_date + \
